...
function representatives(){return [...new Set([...db.clientes.map(x=>x.representante),...db.pedidos.map(x=>x.representante)].filter(Boolean))].sort()}
function currentMonthOrders(){return db.pedidos.filter(p=>pedidoMonth(p)===monthNow())}
function regionOf(p){let uf=norm(p.uf||getClient(p.clienteId)?.uf);const map={SP:"Sudeste",RJ:"Sudeste",MG:"Sudeste",ES:"Sudeste",PR:"Sul",SC:"Sul",RS:"Sul",BA:"Nordeste",SE:"Nordeste",AL:"Nordeste",PE:"Nordeste",PB:"Nordeste",RN:"Nordeste",CE:"Nordeste",PI:"Nordeste",MA:"Nordeste",GO:"Centro-Oeste",MT:"Centro-Oeste",MS:"Centro-Oeste",DF:"Centro-Oeste",AM:"Norte",PA:"Norte",AC:"Norte",RO:"Norte",RR:"Norte",AP:"Norte",TO:"Norte"};return map[uf]||"Não informado"}

function render(){renderClientes();renderPedidos();renderFollow();renderAlerts();renderMetas();fillSelectors();renderDashboard();checkNotifications()}
function renderDashboard(){
 const orders=currentMonthOrders(),total=orders.reduce((s,p)=>s+Number(p.valor||0),0);
 $("kVendas").textContent=money(total);$("kPedidos").textContent=orders.length;$("kAlertas").textContent=db.alertas.filter(a=>!a.done&&a.data<=today()).length;
 const regions={};orders.forEach(p=>{let r=regionOf(p);regions[r]=(regions[r]||0)+Number(p.valor||0)});const sorted=Object.entries(regions).sort((a,b)=>b[1]-a[1]);$("kRegiao").textContent=sorted[0]?.[0]||"—";
 let max=sorted[0]?.[1]||1;$("regiaoRanking").innerHTML=sorted.length?sorted.map(([r,v])=>`<div class="barrow"><div class="barlabel"><b>${esc(r)}</b><span>${money(v)}</span></div><div class="bartrack"><div class="barfill" style="width:${Math.max(3,v/max*100)}%"></div></div></div>`).join(""):'<p class="muted">Importe pedidos para visualizar as regiões.</p>';
 const reps=representatives();$("repRanking").innerHTML=reps.length?reps.map(rep=>{let sold=orders.filter(p=>norm(p.representante)===norm(rep)).reduce((s,p)=>s+Number(p.valor||0),0),goal=Number(db.metas[rep]||0),left=Math.max(goal-sold,0),pct=goal?Math.min(sold/goal*100,100):0;return `<div class="rep-card"><div class="barlabel"><b>${esc(rep)}</b><span>${goal?`${pct.toFixed(1)}%`:"Sem meta"}</span></div><div class="muted">Vendido: ${money(sold)} • Falta: ${goal?money(left):"Defina a meta"}</div>${goal?`<div class="bartrack" style="margin-top:7px"><div class="barfill" style="width:${pct}%"></div></div>`:""}</div>`}).join(""):'<p class="muted">Nenhum representante encontrado.</p>';
 const aa=db.alertas.filter(a=>!a.done&&a.data<=today()).sort((a,b)=>a.data.localeCompare(b.data)).slice(0,6);$("painelAlertas").innerHTML=aa.length?aa.map(alertHtml).join(""):'<p class="muted">Nenhum alerta para hoje.</p>';
 const ff=db.followups.filter(f=>!f.done).sort((a,b)=>a.data.localeCompare(b.data)).slice(0,6);$("painelFollow").innerHTML=ff.length?ff.map(followHtml).join(""):'<p class="muted">Nenhum follow-up pendente.</p>';
}
...
function renderPedidos(){
 ...
 let total=arr.reduce((s,p)=>s+Number(p.valor||0),0);$("pTotal").textContent=money(total);$("pQtd").textContent=arr.length;$("pTicket").textContent=money(arr.length?total/arr.length:0);arr.sort((a,b)=>(b.data||"").localeCompare(a.data||""));
...
function renderMetas(){let reps=representatives(),orders=currentMonthOrders();$("metasList").innerHTML=reps.map(rep=>{let sold=orders.filter(p=>norm(p.representante)===norm(rep)).reduce((s,p)=>s+Number(p.valor||0),0),goal=Number(db.metas[rep]||0),left=Math.max(goal-sold,0);return `<div class="card" style="margin-bottom:10px"><b>${esc(rep)}</b><div class="muted">Meta: ${money(goal)} • Vendido: ${money(sold)} • Falta: ${goal?money(left):"Meta não definida"}</div></div>`}).join("")||'<div class="card muted">Importe clientes/pedidos com representantes.</div>'}
function fillSelectors(){let reps=representatives(),opts='<option value="">Todos os representantes</option>'+reps.map(r=>`<option>${esc(r)}</option>`).join("");let cur=$("repPedido").value;$("repPedido").innerHTML=opts;$("repPedido").value=cur;$("metaRep").innerHTML=reps.map(r=>`<option>${esc(r)}</option>`).join("");$("fCliente").innerHTML=db.clientes.map(c=>`<option value="${c.id}">${esc(c.codigo)} — ${esc(c.razao)}</option>`).join("")}
...
function primeiroNomeRazao(c){let r=String(c?.razao||"").trim();return r?r.split(/\s+/)[0]:""}
function openWhats(id){
 const c=getClient(id);if(!c)return;
 const phones=[c.tel1,c.tel2].filter(Boolean).map((p,i)=>({raw:p,clean:cleanPhone(p),label:`Telefone ${i+1}: ${p}`})).filter(x=>x.clean);
 if(!phones.length)return alert("Este cliente não possui telefone cadastrado.");
 $("wClienteId").value=id;$("wClienteNome").textContent=`${c.codigo||""} — ${c.razao||""}`;
 $("wTelefone").innerHTML=phones.map(x=>`<option value="${x.clean}">${esc(x.label)}</option>`).join("");
 $("wModelo").value="follow";aplicarModeloWhats();$("whatsModal").classList.add("open")
}
function aplicarModeloWhats(){
 const c=getClient($("wClienteId").value);if(!c)return;
 const nome=c.razao||"cliente", modelo=$("wModelo").value;
 const modelos={
  follow:`Olá! Tudo bem? Estou entrando em contato para dar continuidade ao nosso atendimento comercial. Posso te ajudar em algo hoje?`,
  parado:`Olá! Tudo bem? Percebi que faz um tempo desde a última compra e queria saber se posso te ajudar com alguma reposição, novidade ou condição comercial.`,
  catalogo:`Olá! Tudo bem? Temos novidades no catálogo e queria compartilhar algumas opções que podem fazer sentido para sua empresa. Posso te enviar?`,
  pedido:`Olá! Tudo bem? Estou entrando em contato para acompanhar seu pedido/retorno comercial. Se precisar de qualquer informação, estou à disposição.`,
  personalizada:""
 };
 $("wMensagem").value=modelos[modelo]||""
}
function abrirWhatsApp(){
 const phone=$("wTelefone").value,msg=$("wMensagem").value.trim();
 if(!phone)return alert("Telefone inválido.");
 const url=`https://wa.me/${encodeURIComponent(phone)}?text=${encodeURIComponent(msg)}`;
 window.open(url,"_blank","noopener")
}

async function enableNotifications(){if(!("Notification" in window))return alert("Este navegador não oferece notificações.");let p=await Notification.requestPermission();$("notifyBtn").textContent=p==="granted"?"🔔 Notificações ativadas":"🔕 Notificações bloqueadas";checkNotifications()}
...
function importPedidos(){
 ...
 let r=new FileReader();r.onload=e=>{try{let wb=XLSX.read(e.target.result,{type:"array",cellDates:true}),existing=new Map(db.pedidos.map(p=>[String(p.ordem),p])),count=0;
 wb.SheetNames.forEach(sn=>{let rows=XLSX.utils.sheet_to_json(wb.Sheets[sn],{defval:""});rows.forEach(row=>{let ordem=pick(row,["ORDEM","PEDIDO","Nº PEDIDO","NUMERO PEDIDO","NÚMERO PEDIDO"]);if(ordem==="")return;let codigo=pick(row,["CODIGO CLIENTE","CÓDIGO CLIENTE","COD CLIENTE","CODIGO","CÓDIGO"]);let cli=codigo!==""?findClientByCode(codigo):null;let nome=pick(row,["NOME DO CLIENTE","CLIENTE","RAZAO CLIENTE","RAZÃO CLIENTE"])||cli?.razao||"";let p={id:existing.get(String(ordem))?.id||uid(),ordem,codigoCliente:codigo||cli?.codigo||"",cliente:nome,data:excelDate(pick(row,["DIGITAÇÃO","DIGITACAO","DATA","DATA PEDIDO","EMISSÃO","EMISSAO"])),representante:pick(row,["REPRESENTANTE","VENDEDOR"])||cli?.representante||"",valor:Number(pick(row,["VALOR LÍQUIDO","VALOR LIQUIDO","VALOR","TOTAL","VLR LIQUIDO"])||0),cidade:pick(row,["CIDADE"])||cli?.cidade||"",uf:pick(row,["UF","ESTADO"])||cli?.uf||"",status:pick(row,["STATUS","ETAPA"]),aba:sn};existing.set(String(ordem),p);count++})});
 db.pedidos=[...existing.values()];$("pedidosMsg").textContent=`Importação concluída: ${count} registros lidos e ${db.pedidos.length} pedidos únicos no sistema.`;save()}catch(err){alert("Não consegui importar os pedidos: "+err.message)}};r.readAsArrayBuffer(f)
}
function exportExcel(){let wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(db.clientes),"Clientes");XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(db.pedidos),"Pedidos");XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(db.followups.map(f=>({cliente:getClient(f.clienteId)?.razao,data:f.data,assunto:f.texto,status:f.done?"Concluído":"Pendente"}))),"Follow-ups");XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(db.alertas.map(a=>({cliente:getClient(a.clienteId)?.razao,data:a.data,prioridade:a.prioridade,motivo:a.texto,status:a.done?"Concluído":"Pendente"}))),"Alertas");XLSX.writeFile(wb,"Gestor_Comercial.xlsx")}
function backupDados(){const pacote={aplicativo:"Gestor Comercial",versao:1,criadoEm:new Date().toISOString(),dados:db};const blob=new Blob([JSON.stringify(pacote,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`Backup_Gestor_Comercial_${today()}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000);alert("Backup criado com sucesso.")}
function restaurarBackup(event){const f=event.target.files[0];if(!f)return;if(!confirm("A restauração substituirá os dados atuais. Deseja continuar?")){event.target.value="";return}const r=new FileReader();r.onload=e=>{try{const pacote=JSON.parse(e.target.result),dados=pacote.dados||pacote;if(!dados||!Array.isArray(dados.clientes))throw new Error("Arquivo inválido");db={clientes:dados.clientes||[],pedidos:dados.pedidos||[],followups:dados.followups||[],alertas:dados.alertas||[],metas:dados.metas||{}};localStorage.setItem(KEY,JSON.stringify(db));render();alert("Backup restaurado com sucesso.")}catch(err){alert("Não foi possível restaurar este backup.")}event.target.value=""};r.readAsText(f)}
